...
class Program
{
    public static void Heapify(int[] arr, int n, int i)
    {
        ...
        //nếu chỉ số left bé hơn n phần tử và giá trị ở left > giá trị ở largest
            largest = left;
        if (right < n && arr[right] > arr[largest])
            //nếu chỉ số right bé hơn n phần tử và giá trị ở right > giá trị ở largest
            largest = right;
        if (largest != i) //nếu largest != i thì có phần tử lớn hơn phần tử ở chỉ số largest
        {
            ...
            arr[i] = arr[largest]; 
            arr[largest] = tmp;
            //Heapify kiểm tra các nốt trung gian
